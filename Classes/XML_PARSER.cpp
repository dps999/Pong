...
void XML_PARSER::Parse_Objects(IXMLDOMNodePtr pObject)
{
        ...
        if(Is_MSXML_Node(pObject) != NODE_ELEMENT)
           return;
...
{  // Now you can handle the text value on the real node directly if you want
   ...
   if(Is_MSXML_Node(this->CurrentNode) == NODE_TEXT || Is_MSXML_Node(this->CurrentNode) == NODE_CDATA_SECTION)
   ...
   if(this->CurrentNode->hasChildNodes())
   {
           ...
           {       // Find it's a NODE TEXT
                   //
                   if(this->Is_MSXML_Node(pChild) == NODE_TEXT)
...
HRESULT XML_PARSER::Is_MSXML_Node(IXMLDOMNodePtr pChild)
{       if(pChild == NULL) return NULL;
        ...
        if(pChild->nodeType == NODE_TEXT) return NODE_TEXT;
        if(pChild->nodeType == NODE_CDATA_SECTION) return NODE_CDATA_SECTION;
        if(pChild->nodeType == NODE_PROCESSING_INSTRUCTION ) return NODE_PROCESSING_INSTRUCTION;
        return NODE_ELEMENT;
}

void XML_PARSER::Grab_Node_Informations(IXMLDOMNodePtr pChild)
{
        if(this->Is_MSXML_Node(pChild) == NODE_ELEMENT)
        {       // It's a node
                //
        ...
        else if (this->Is_MSXML_Node(pChild) == NODE_TEXT)
        {   // Tag is #TEXT#
            //
            Tag.Empty();
            Tag = "<#TEXT#>";
        ...
        else if (this->Is_MSXML_Node(pChild) == NODE_CDATA_SECTION)
        {   // Tag is #CDATA#
            //
            Tag.Empty();
            Tag = "<#CDATA#>";
        ...
        else if (this->Is_MSXML_Node(pChild) == NODE_PROCESSING_INSTRUCTION)
        {  /* Do nothing here */ } 
...
bool XML_PARSER::_add_lastchild(IXMLDOMNodePtr newNode)
{       // Attach the Node to the document
        //
        if(CurrentNode != NULL)
        {       if(Is_MSXML_Node(this->CurrentNode) != NODE_ELEMENT) return false;
                this->ChainTag_Add( this->Get_CurrentTag() );
                newNode = CurrentNode->appendChild(newNode);
        }
        else if(m_pDocRoot != NULL)
        {       if(Is_MSXML_Node(this->m_pDocRoot) != NODE_ELEMENT) return false;
...
bool XML_PARSER::_add_firstchild(IXMLDOMNodePtr newNode)
...
bool XML_PARSER::_add_before(IXMLDOMNodePtr newNode)
{       if(this->Is_Root())
...
bool XML_PARSER::_add_after(IXMLDOMNodePtr newNode)
...
{
        // Nodes members
        IXMLDOMElementPtr CurrentElementNode = CurrentNode;
...
bool XML_PARSER::Set_TextValue(LPCTSTR TextValue)
{
        ...
        if(this->Is_MSXML_Node(this->CurrentNode) == NODE_CDATA_SECTION)
        {
        ...
        //
        if(this->Is_MSXML_Node(this->CurrentNode) == NODE_ELEMENT)
...
{       // Child node
        ...
        if(Is_MSXML_Node(this->CurrentNode) != NODE_ELEMENT)
                return false;
...
{       // Sibling node
        ...
        if(Is_MSXML_Node(this->CurrentNode) != NODE_ELEMENT && Is_MSXML_Node(this->CurrentNode) != NODE_CDATA_SECTION)
                return false;
...
bool XML_PARSER::Go_to_Descendant(LPCTSTR NodeName)
{
        ...
        //
        IXMLDOMElementPtr pCurrent = this->CurrentNode;
...
bool XML_PARSER::Go_to_Parent(LPCTSTR NodeName)
{
        ...
        //
        IXMLDOMNodePtr pParent = NULL;
        IXMLDOMNodePtr oldCurrent = this->CurrentNode;
...
bool XML_PARSER::Get_Header(LPCTSTR header,LPCTSTR name,CString & res)
{
      ...
      while(pChild != NULL)
      {
          if(pChild->nodeType != NODE_PROCESSING_INSTRUCTION) break;
